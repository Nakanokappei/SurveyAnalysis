...
// drilling from a weekday into its 個票一覧 (記入日 + 抜粋, PII hidden). Modelled on TimeSliceControl but
// simpler — the weekday list is a single level, so the only navigation is the clickable breadcrumb
// (曜日 ＞ 月曜日); there is no 戻る button and no per-level title. Built from layout containers only
// (TableLayoutPanel rows, Dock/Anchor/AutoSize) per the layout rules; no explicit coordinates. The view
// model mutates its collections in place, so the control re-reads everything right after invoking a
// command (drill / breadcrumb / period change), which all run synchronously.
internal sealed class WeekdaySliceControl : UserControl
{
    private readonly WeekdaySliceViewModel _vm;
    ...
    // Header: title + 件数 summary on the left, the 集計期間 picker on the right.
    private Control BuildHeader()
    {
        ...
        titles.Controls.Add(new Label { Text = "曜日", AutoSize = true, ForeColor = Theme.TitleText, Font = Theme.Font(17f, FontStyle.Bold), Margin = new Padding(0) });
    ...
    {
        // Clicking a weekday row drills into its 個票 (the source AnalysisRow is in the row Tag).
    ...
    // Re-reads the whole view model after any navigation: 件数 summary, the breadcrumb bar, which grid is
    // shown, and that grid's rows.
